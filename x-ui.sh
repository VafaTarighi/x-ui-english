...
uninstall() {
    ...
    echo ""
    echo -e "The uninstallation is succesful, if you want to delete this script, run ${green}rm /usr/bin/x-ui -f${plain} after exiting the script to delete"
    echo ""
...
disable() {
    ...
    if [[ $? == 0 ]]; then
        ...
        LOGE "x-ui failed to cancel the boot self-start"
    fi
...
show_usage() {
    ...
    echo "x-ui enable       - Set x-ui to auto-enable"
    echo "x-ui disable      - Cancel x-ui auto-start at boot"
    echo "x-ui log          - View the x-ui log"
...
————————————————
  ${green}13.${plain} set up x-ui Auto-start
  ${green}14.${plain} cancel x-ui boot auto-start
————————————————
  ${green}15.${plain} 一key install bbr (latest kernel)
